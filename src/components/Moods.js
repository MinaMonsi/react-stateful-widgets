...
export default function Moods() {
  /* STEP 1 Created mood state*/
  const [mood, setMood] = useState(initialMood);

  const makeHappy = () => {
    /* STEP 4 - setMood to happy*/setMood(happyMood)
  };
  const makeSad = () => {
    /* STEP 5 - setMood to sad*/setMood(sadMood)
  };
  const reset = () => {
    /* STEP 6 - setMood to initialMood*/setMood(initialMood)
  };

  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: mood === makeHappy? 'royalblue' : 'crimson', /* STEP 2 - Ternary expression written */
  };
